;( function ( window, document, $, THREE ) {
    ...
    function init () {
		...
		*/
		...
		astronaut = new THREE.Mesh( new THREE.CubeGeometry( 200, 700, 400 ), new THREE.MeshNormalMaterial() );
		astronaut.position.x = -300;
		astronaut.position.y = -150;
        astronaut.receiveShadow = true;
		astronaut.castShadow    = true;
		astronaut.material.colorWrite = false;
		astronaut.material.depthWrite = false;
		scene.add( astronaut );
    ...
    function onResize () {
		...
		var img = document.getElementById('main-container'),
			style = img.currentStyle || window.getComputedStyle(img, false),
			imageSrc = style.backgroundImage.replace(/url\((['"])?(.*?)\1\)/gi, '$2').split(',')[0];
		var image = new Image();
		image.src = imageSrc;
		var width = image.width,
			height = image.height;
		camera.zoom = 0.75 * Math.max(1, height / (window.innerHeight / window.innerWidth * width));
